...
async function parseHeader(blob) {
    ...
    //	dataOffset,
    //});
    const dirEntries = await parseDirEntry(blob, Number(dirEntryOffset), Number(dirEntryLength));
    //console.log(dirEntries);
    const fileEntries = await parseFileEntry(blob, Number(fileEntryOffset), Number(fileEntryLength));
    //console.log(fileEntries);
    for (const fileEntry of fileEntries) {
        let { parentOffset } = fileEntry;
        const pathParts = [fileEntry.name];
        while (parentOffset !== 0) {
            const parent = dirEntries.find((dir) => dir.offset === parentOffset);
            if (!parent)
                break;
